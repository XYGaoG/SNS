...
class ATT_HGCN(nn.Module):
	...
	def __init__(self, net_schema, layer_shape, label_keys, type_fusion='att', type_att_size=64):
		...
		self.transfer = nn.ParameterDict()
		self.transfer2 = nn.ParameterDict()
		for k in net_schema:
			self.transfer[k] = nn.Linear(layer_shape[2][k], layer_shape[2][k])
			self.transfer2[k] = nn.Linear(layer_shape[2][k], layer_shape[2][k])


		self.semantic_classifier = nn.Linear(layer_shape[2][label_keys[0]], layer_shape[-1][label_keys[0]])
	...
	def forward(self, ft_dict, adj_dict):
		...
		embd2 = {}
		for k in embd:
			embd1[k]=F.elu(self.transfer[k](embd[k].clone()))
			embd2[k]=F.elu(self.transfer2[k](embd[k].clone()))

		return logits, embd1, embd2, attention_list
	...
	def seman_cla(self, x_dict):
		y_dict = F.elu(self.semantic_classifier(x_dict))
		return y_dict