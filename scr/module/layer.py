...
class HeteAggregateLayer(nn.Module):
	...
	def __init__(self, curr_k, nb_list, in_layer_shape, out_shape, type_fusion, type_att_size):
		...
		# self.temp_weight = torch.FloatTensor(in_layer_shape[curr_k], out_shape)
		# self.w_self = nn.Parameter(self.temp_weight)
		# nn.init.xavier_uniform_(self.w_self.data, gain=1.414)

		self.w_self = torch.nn.Linear(in_layer_shape[curr_k], out_shape, bias=False)
		nn.init.xavier_uniform_(self.w_self.weight, gain=1.414)
	...
	def forward(self, x_dict, adj_dict):
		...
		self_ft = self.w_self(x_dict[self.curr_k])
		nb_ft_list = [self_ft]
